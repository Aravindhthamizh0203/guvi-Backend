...
router.get('/verifyemail/:id', express.json(), async function (req, res) {
  const { id } = req.params;
...
router.post('/resetpassword', express.json(), async function (req, res) {
  const data = req.body;
  ...
  if (getData) {
    ...
    res.status(200).send({ message: 'successfull', mail: mail })
  } else {
...
router.get('/resetpassword/:string', express.json(), async function (req, res) {
  const { string } = req.params;
  const getResult = await checkString(string);
  if (getResult) {
    res.status(200).send({ message: "present", email: getResult.email })
  } else {
    res.status(404).send({ message: "abscent" })
  }
})

//to reset and password and delete from the database.
router.post('/changepassword/:string', express.json(), async function (req, res) {
  const { string } = req.params;
  const data = req.body;
...
router.get('/verifyToken', auth, express.json(), async function (req, res) {
...
router.post('/login', express.json(), async function (req, res) {
  ...
  if (userFromDB == null) {
    ...
    if (userFromDB.emailVerified == 'yes') {
      ...
      const mail = await sendEmail(email, "verification token", `${API}/emailverify/${token}`)
      res.status(406).send({ message: "verification pending" })
    }
  }
})
